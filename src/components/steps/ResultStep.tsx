...
`;
    ...
    if (printWindow) {
      printWindow.document.write(`
        <html>
          ...
          <body>
            <h1>CryptoKey Secure Backup</h1>
            <p><strong>Hash:</strong> ${result.hash}</p>
            <div class="box">
              <strong>Ciphertext:</strong><br/><br/>
              ${result.ciphertext}
            </div>
            <p class="footer">Decrypt at https://CryptoKey.im | Generated on ${new Date().toLocaleString()}</p>
  ...
  const handleCopy = async () => {
    const textToCopy = `Ciphertext: ${result.ciphertext}\nHash: ${result.hash}`;
    try {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className={`space-y-4 ${mode === 'public' ? 'opacity-20 pointer-events-none blur-sm select-none' : ''} transition-all duration-500`}>
          ...
          <div className="bg-gradient-to-br from-primary/10 to-purple-500/10 border border-primary/20 rounded-xl p-6 relative overflow-hidden group hover:border-primary/40 transition-all">
            ...
            <p className="text-xs text-primary/80 font-medium">
              This is the ONLY row containing your data. All others are fake.
            </p>
          </div>
          ...
          {mnemonic && (
            <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-6">
               <h3 className="text-xs font-bold text-slate-500 mb-2 uppercase tracking-wider">Original Mnemonic</h3>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-4">
            <label className="text-xs font-bold text-slate-500 mb-1 block">Hash</label>
          ...
          <Button 
             ...
             title={!features.allowDualAuth ? "Pro Feature: Upgrade to Access" : ""}
          >
             <span className="flex items-center justify-center gap-2">
               Generate Dual Auth Shards
