...
export function ImageStego({ initialSecret, onExtract: _onExtract }: SteganographyToolProps) {
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    filesToProcess.forEach(file => {
        ...
        reader.onload = (event) => {
            const img = new Image();
            img.onload = () => {
                // Capacity Check (Optional Warning)
                const capacityBytes = Math.floor((img.width * img.height * 3) / 8);
                const capacityMB = (capacityBytes / (1024 * 1024)).toFixed(2);
                
                // We no longer limit to 1.5MB hardcoded. 
                // Instead, we trust the capacity calculation during embedding.
                console.log(`Loaded carrier: ${file.name}, Capacity: ${capacityMB} MB`);
