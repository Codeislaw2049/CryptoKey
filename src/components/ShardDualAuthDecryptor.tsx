...
const ShardDualAuthDecryptor: React.FC<ShardDualAuthDecryptorProps> = ({ initialShards }) => {
  ...
  const handleStreamScanComplete = (chunks: Map<number, string>, hash?: string) => {
    ...
    setProcessingStatus(t('dualAuthDecryptor.mergeSuccess'));
    ...
    <div className="space-y-6">
      ...
      <div className="border p-4 rounded-lg bg-slate-900/50 border-indigo-500/30 relative overflow-hidden">
        ...
        <h3 className="font-semibold flex items-center gap-2 text-white relative z-10">
          ...
          {t('dualAuthDecryptor.step1.title')}
        </h3>
        <p className="text-sm text-slate-400 mb-4 relative z-10">
          {t('dualAuthDecryptor.step1.instruction')}
        </p>
        ...
        <div className="flex items-center gap-4 relative z-10">
          ...
          >
            ...
            {t('dualAuthDecryptor.scanStream')}
          </Button>
          ...
          <Button variant="outline" className="relative border-white/20 hover:bg-white/10 text-white">
            ...
            />
            {t('dualAuthDecryptor.selectImages')}
          </Button>
          ...
          {(shards.size > 0 || failedFiles.length > 0) && (
            ...
            >
              {t('dualAuthDecryptor.clearReset')}
            </Button>
          )}
          ...
          <span className="text-sm font-mono text-slate-300">
            {processingStatus ? (
              ...
              <span className={shards.size === totalShards ? "text-green-400" : "text-yellow-400"}>
                 {t('dualAuthDecryptor.progress', { count: shards.size, total: totalShards || '?' })}
              </span>
            )}
          </span>
        </div>
        ...
        {failedFiles.length > 0 && (
          <div className="mt-2 text-xs text-red-400 bg-red-900/20 p-2 rounded border border-red-800">
            <p className="font-bold flex items-center gap-1"><AlertTriangle className="w-3 h-3"/> {t('dualAuthDecryptor.failedFiles', { count: failedFiles.length })}</p>
        ...
        {totalShards > 0 && missingIndices.length > 0 && (
           <div className="mt-2 text-xs text-yellow-400 bg-yellow-900/20 p-2 rounded border border-yellow-800">
             {t('dualAuthDecryptor.missingParts', { parts: missingIndices.join(', ') })}
           </div>
        )}
        ...
        {mergedData && (
          <div className="text-green-400 text-sm flex items-center gap-1">
            <CheckCircle className="w-4 h-4" /> {t('dualAuthDecryptor.mergeSuccess')}
          </div>
        )}
      </div>
      ...
      <div className="space-y-4 border p-4 rounded-lg bg-gray-50/10 border-white/10">
        <h3 className="font-semibold flex items-center gap-2 text-white">
          ...
          {t('dualAuthDecryptor.step2.title')}
        </h3>
        <p className="text-sm text-slate-400">
          {t('dualAuthDecryptor.step2.description')}
        </p>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {/* Key A Input */}
          <div className="space-y-2">
            <label className="block text-xs font-medium text-slate-300">{t('dualAuthDecryptor.keyA.label')}</label>
            ...
            <div className="relative">
               <Button variant={secretA ? "primary" : "outline"} size="sm" className={`w-full justify-center ${!secretA && 'text-slate-300 border-white/20 hover:bg-white/10'}`}>
                ...
                {secretA ? t('dualAuthDecryptor.keyA.loaded') : t('dualAuthDecryptor.keyA.upload')}
              </Button>
            </div>
            ...
            <Input
              type="password"
              placeholder={t('dualAuthDecryptor.keyA.placeholder')}
              value={secretA}
            ...
            <p className="text-[10px] text-slate-500 italic">
               {t('dualAuthDecryptor.lostQrHelp')}
            </p>
          </div>

          {/* Key B Input */}
          <div className="space-y-2">
            <label className="block text-xs font-medium text-slate-300">{t('dualAuthDecryptor.keyB.label')}</label>
            ...
            <div className="relative">
               <Button variant={secretB ? "primary" : "outline"} size="sm" className={`w-full justify-center ${!secretB && 'text-slate-300 border-white/20 hover:bg-white/10'}`}>
                ...
                />
                <ScanLine className="w-4 h-4 mr-2" />
                {secretB ? t('dualAuthDecryptor.keyB.loaded') : t('dualAuthDecryptor.keyB.upload')}
              </Button>
            </div>
            ...
            {/* Manual Input */}
             <Input
              type="password"
              placeholder={t('dualAuthDecryptor.keyB.placeholder')}
              value={secretB}
              autoComplete="new-password"
      ...
      <div className="space-y-4 border p-4 rounded-lg bg-gray-50/10 border-white/10">
        <h3 className="font-semibold flex items-center gap-2 text-white">
          ...
          {t('dualAuthDecryptor.step3.title')}
        </h3>
        <p className="text-sm text-slate-400">
          {t('dualAuthDecryptor.step3.description')}
        </p>

        <div className="grid grid-cols-2 gap-4">
          <Input
            placeholder={t('dualAuthDecryptor.codeA.placeholder')}
            value={codeA}
          ...
          />
          <Input
            placeholder={t('dualAuthDecryptor.codeB.placeholder')}
            value={codeB}
        ...
        <div className="space-y-2">
          ...
          >
            {t('dualAuthDecryptor.decryptButton')}
          </Button>
          {(!secretA || !secretB) && (
            <p className="text-xs text-center text-red-400 animate-pulse">
              {t('dualAuthDecryptor.keysRequired')}
            </p>
          )}
        </div>
      </div>
      ...
      {decryptedResult && (
        <div className="bg-slate-800 p-6 rounded-xl border border-indigo-500/30 space-y-4 animate-in fade-in slide-in-from-bottom-4">
          <div className="flex items-center gap-2 mb-2">
            ...
            <h3 className="text-lg font-bold text-white">{t('dualAuthDecryptor.success.title')}</h3>
          </div>

          <div>
            <div className="flex justify-between items-end mb-1">
              <label className="text-xs font-semibold text-indigo-400 uppercase tracking-wider block">
                 {t('dualAuthDecryptor.result.mnemonicLabel')}
              </label>
              <button
            ...
            {isBookCipherIndices(decryptedResult.mnemonic) && (
               <div className="mt-2 text-xs text-yellow-400 bg-yellow-900/20 p-2 rounded border border-yellow-800 flex items-start gap-2">
                 ...
                 <span>
                   {t('dualAuthDecryptor.bookCipherNote')}
                 </span>
               </div>
            )}
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-xs font-semibold text-indigo-400 uppercase tracking-wider block mb-1">{t('dualAuthDecryptor.result.passwordLabel')}</label>
              <div className="bg-slate-950 p-3 rounded border border-slate-700 font-mono text-sm text-white">
                {decryptedResult.password || <span className="text-slate-600 italic">{t('common.none')}</span>}
              </div>
            </div>
            <div>
              <label className="text-xs font-semibold text-indigo-400 uppercase tracking-wider block mb-1">{t('dualAuthDecryptor.result.lineLabel')}</label>
              <div className="bg-slate-950 p-3 rounded border border-slate-700 font-mono text-sm text-white">
          ...
          {decryptedResult.fullRows && decryptedResult.fullRows.length > 0 && (
             <div className="pt-4 border-t border-slate-700/50">
               <label className="text-xs font-semibold text-slate-500 block mb-1">{t('dualAuthDecryptor.result.fullContent')}</label>
          ...
          {decryptedResult.ciphertext && decryptedResult.ciphertext !== decryptedResult.mnemonic && (
             <div className="pt-4 border-t border-slate-700/50">
               <label className="text-xs font-semibold text-slate-500 block mb-1">{t('dualAuthDecryptor.result.rawCiphertext')}</label>
               <textarea 
                 readOnly 
