...
export function WalletGenerator({ onBackup, onBackupToBook }: WalletGeneratorProps) {
  ...
  const handleImport = () => {
    ...
    if (!validation.isValid) {
      setImportError(validation.error || t('wallet.error.invalidMnemonic'));
      return;
    }
          ...
          >
            ...
            <div className="bg-slate-800/50 p-6 rounded-xl border border-slate-700 relative overflow-hidden group">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-lg font-bold text-slate-200">{t('wallet.mnemonic.title')}</h3>
            ...
            <div className="bg-slate-800/50 p-6 rounded-xl border border-slate-700 space-y-6">
              <div className="flex justify-between items-center">
                <h3 className="text-lg font-bold text-slate-200">{t('wallet.details.title')}</h3>
                <button
                ...
                >
                  ...
                  {showSensitive ? t('wallet.details.hideKeys') : t('wallet.details.showKeys')}
                </button>
              </div>
              ...
              <div className="flex flex-col md:flex-row gap-6">
                <div className="flex-1 space-y-4">
                  <div className="space-y-1">
                    <label className="text-xs text-slate-500 uppercase font-bold tracking-wider">{t('wallet.details.ethAddress')}</label>
                    ...
                    <p className="text-xs text-slate-500 flex items-center gap-1">
                      ...
                      {t('wallet.details.path', { path: walletDetails?.path })}
                    </p>
                  </div>
                    ...
                    >
                      <div className="space-y-1">
                        <label className="text-xs text-red-400 uppercase font-bold tracking-wider flex items-center gap-1">
                          <Key size={12} /> {t('wallet.details.privateKey')}
                        </label>
                      ...
                      <div className="space-y-1">
                        <label className="text-xs text-slate-500 uppercase font-bold tracking-wider">{t('wallet.details.publicKey')}</label>
            ...
            <div className="bg-yellow-500/10 border border-yellow-500/20 p-4 rounded-lg flex gap-3 items-start">
              ...
              <div className="text-sm text-yellow-200/80 space-y-1">
                <p className="font-bold text-yellow-500">{t('wallet.security.title')}</p>
                <p>
                  {t('wallet.security.line1')}{' '}
                  <strong>{t('wallet.security.never')}</strong>{' '}
                  {t('wallet.security.line2')}{' '}
                  {t('wallet.security.line3')}
                </p>
              </div>
            </div>
